...
create(Pid) ->
    ...
    {ok, ?DOMAIN_DOMAIN, D, _Pid} = nkdomain:find(root, ?DOM),
    {ok, #{
        <<"_is_enabled">> := true,
...
base_msgs(Pid, Ref) ->
    ...
    {ok, _, C1, _} = nkdomain:find(root, <<"/chattest/conversations/conv1">>),
    {ok, _, C2, _} = nkdomain:find(root, <<"/chattest/conversations/conv2">>),
    {ok, _, _C3, _} = nkdomain:find(root, <<"/chattest/conversations/conv3">>),
...
session1(Pid, _Ref) ->
    ...
    {ok, _, _, _SPid} = nkdomain:find(root, S),
