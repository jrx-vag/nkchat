...
-module(nkchat_session_obj_type_view).
...
%%    case table_filter(maps:to_list(Filter), #{timezone_offset => Offset}, #{type=>?CHAT_SESSION}) of
%%        {ok, Filters} ->
...
%%                size => Size
%%            },
%%            SubDomainsFilterId = nkdomain_admin_util:make_type_view_subfilter_id(?CHAT_SESSION, Path),
...
%%    Base = nkdomain_admin_util:table_entry(?CHAT_SESSION, Entry, Pos),
%%    Data = Base#{
%%    },
...
get_agg_name(Field, Path, Session) ->
    nkdomain_admin_util:get_agg_name(Field, ?CHAT_SESSION, Path, Session).


%% @private
get_agg_srv_id(Path, Session) ->
    nkdomain_admin_util:get_agg_srv_id(?CHAT_SESSION, Path, Session).
