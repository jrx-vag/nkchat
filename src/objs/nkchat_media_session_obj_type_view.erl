    ...
    Spec = #{
        ...
        columns => [
            ...
            #{
                ...
                sort => true,
                options => nkdomain_admin_util:get_agg_name(<<"domain_id">>, ?MEDIA_SESSION, Session)
            },
            #{
                id => service,
                type => text,
                ...
                sort => true,
                options => nkdomain_admin_util:get_agg_srv_id(?MEDIA_SESSION, Session)
            },
            #{
                id => obj_name,
                type => text,
            ...
            #{
                ...
                sort => true,
                options => nkdomain_admin_util:get_agg_name(<<"created_by">>, ?MEDIA_SESSION, Session),
                is_html => true % Will allow us to return HTML inside the column data
            },
            #{
                id => created_time,
...
table_data(#{start:=Start, size:=Size, sort:=Sort, filter:=Filter}, _Opts, #admin_session{domain_id=DomainId}) ->
    ...
    case table_filter(maps:to_list(Filter), #{timezone_offset => Offset}, #{type=>?MEDIA_SESSION}) of
        {ok, Filters} -> 
...
table_iter([Entry|Rest], Pos, Acc) ->
    Base = nkdomain_admin_util:table_entry(?MEDIA_SESSION, Entry, Pos),
    Data = Base#{
    },
