...
-export([object_admin_info/0, object_schema_types/0, object_do_active/1]).
...
-define(INACTIVITY_TIMER, nkchat_app:get(inactivity_timer)).
                ...
                {ok, MemberId, Op2} ->
                    ...
                    case nkdomain_token_obj:create(DomainId, TokenOpts, Op3) of
                        {ok, TokenId, _Pid, _Secs} ->
                            nkchat_conversation:added_invitation(ConvId, UserId, MemberId, TokenId),
                            {ok, TokenId};
                        {error, Error} ->
                            {error, Error}
                    end;
